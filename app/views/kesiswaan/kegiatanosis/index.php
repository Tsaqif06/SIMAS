        ...
        <div class="modal-body">
          <form enctype="multipart/form-data" action="<?= BASEURL; ?>/kegiatanosis/tambahData" method="post">
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control" id="kegiatan_kegiatanOsis" name="kegiatan_kegiatanOsis" placeholder="" required />
            </div>

            <div class="form-group">
              <label for="exampleInputEmail1">Tanggal Kegiatan</label>
              <input type="date" class="form-control" id="tanggal_kegiatanOsis" name="tanggal_kegiatanOsis" placeholder="" required />
            </div>
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control" id="deskripsi_kegiatanOsis" name="deskripsi_kegiatanOsis" placeholder="" required />
            </div>
        </div>
  ...
  <div class="row">
    <div class="col-lg-8">
      <div class="slider" style="width: 100%; height: auto;">
        <?php foreach ($data['kegiatanosis'] as $kegiatanosis) : ?>
          <div class="slide active">
            <img src="images/datafoto/<?= $kegiatanosis['foto'] ?>">
            <div class="info">
              <h4><b><?= $kegiatanosis['kegiatan_kegiatanOsis'] ?></b></h4>
              <h4><b><?= $kegiatanosis['tanggal_kegiatanOsis'] ?></b></h4>
              <h5><?= $kegiatanosis['deskripsi_kegiatanOsis'] ?></h5>
              <a href="<?= BASEURL; ?>/kegiatanosis/ubahData/<?= $kegiatanosis['id'] ?>" data-bs-toggle="modal" data-bs-target="#exampleModalLong" class="tampilModalUbah" data-id="<?= $kegiatanosis['id']; ?>" style="text-align:right; text-decoration: none; color: black;">
                <!-- <button class="button-arounder">  -->
                <span class="material-symbols-outlined"> edit </span>
                <!-- </button> -->
              </a>
              <a href="<?= BASEURL; ?>/kegiatanosis/hapusData/<?= $kegiatanosis['id'] ?>" onclick="return confirm ('Hapus data?')" style="text-align:right; text-decoration: none; color: black;">
                <!-- <button class="button-arounder"> -->
    ...
    $(".tampilModalUbah").click(function() {
      ...
      $.ajax({
        ...
        success: function(data) {
          console.log(data)
          $('#id').val(data.id);
          $('#kegiatan_kegiatanOsis').val(data.kegiatan_kegiatanOsis);
          $('#deskripsi_kegiatanOsis').val(data.deskripsi_kegiatanOsis);
          $('#tanggal_kegiatanOsis').val(data.tanggal_kegiatanOsis);
