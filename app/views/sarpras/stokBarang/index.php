<?php
  include('connect.php');
  // $kategori = mysqli_query($connect, "SELECT kategori FROM stok_barang GROUP BY kategori");
  // $jumlah = mysqli_query($connect, "SELECT SUM(stok) AS jumlah FROM stok_barang GROUP BY kategori");
?>
...
<!-- <div class="main-panel"> -->
        <div class="content-wrapper">
          ...
          <div class="row">
            <div class="col">
              <a class="btn btn-primary" role="button" data-toggle="modal" data-target="#exampleModalLong">ISI FORM PENGAJUAN</a>
            <button type="button" class="btn btn-primary my-3 mx-3 tampilModalImport"
                    data-url="<?= BASEURL ?>/stokBarang" data-bs-toggle="modal" data-bs-target="#modalImport">
                    ...
                    <tbody>
                        ...
                        <tr>
                          ...
                          <td>
                              ...
                              <a href="<?= BASEURL;?>/stokBarang/hapus/<?=$stok['id']?>" onclick="return confirm ('Hapus data?') ">
                              <button class="button-arounder">
  ...
  var stokBarangKategori = {
    labels: [<?php while($row = mysqli_fetch_array($kategori)) {echo '"'.$row['kategori'].'",';} ?>],
    datasets: [{
      label: '# of Votes',
